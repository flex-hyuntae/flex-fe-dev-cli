...
`~/.local/bin` 에 `flex-fe-dev`(TUI), `flex-fe-dev-run`, `flex-fe-dev-open`(legacy) 을 symlink 한다. `~/.local/bin` 이 PATH 에 없으면 `install.sh` 가 안내한다.
...
**host 특수 케이스**: host 는 도메인 remote 가 아니라 MF host 앱이라 `remotes-` 접두사 규칙을 따르지 않는다 → `@flex-apps/host` / `web-applications/host` 로 해석. TUI(`src/core/apps.ts`)와 legacy(`legacy/lib.sh resolve_app_layout`) 둘 다 동일하게 처리한다.
